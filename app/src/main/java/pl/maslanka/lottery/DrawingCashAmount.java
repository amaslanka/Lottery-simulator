...
public class DrawingCashAmount extends AsyncTask<Integer,Integer,List<Integer>> {
    ...
    public DrawingCashAmount(Activity activity) {
        onAttach(activity);
    }
    ...
    protected void onPreExecute() {
        if (activity != null) {
            ((CashAmount) activity).showProgressDialog();

            maxAllNumbersSize = Logic.getMaxAllNumbersPref();
            ((CashAmount) activity).showToastOverLimit(maxAllNumbersSize);
        }
    }
    ...
    protected List<Integer> doInBackground(Integer... ints) {
        ...
        cashAmount = ints[0];
        betNumber = cashAmount/Logic.BET_COST;
        sleepTime = 0;
    ...
    protected void onProgressUpdate(Integer... progress) {
        if (activity == null) {
            ...
            ((CashAmount) activity).updateProgress(progress[0]);
        }
    ...
    protected void onPostExecute(List<Integer> drawnNumbersToPass) {
        if (activity != null) {
            ((CashAmount) activity).hideProgressDialog();
        }

        if (drawnNumbersToPass != null && activity != null) {
            ((CashAmount) activity).startResultActivity(betNumber, drawnNumbersToPass, hits06);
        }
    }
    ...
    private void randomizeYourNumbersAndCheckResults() {
